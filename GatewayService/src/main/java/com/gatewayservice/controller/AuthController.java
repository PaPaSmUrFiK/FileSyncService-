...
@Slf4j
@CrossOrigin(origins = { "http://localhost:5173", "http://localhost:3000", "https://filesync.com",
                "https://app.filesync.com" })
public class AuthController {
                ...
                return authServiceClient.login(request)
                                ...
                                .onErrorResume(e -> {
                                        ...
                                        return Mono.just(ResponseEntity
                                                        ...
                                                        .body(new ErrorResponse(message != null ? message
                                                                        : "Invalid credentials")));
                                });
        }
        ...
        public Mono<ResponseEntity<Void>> changePassword(
                        ...
                        @RequestBody com.gatewayservice.model.ChangePasswordRequest request) {
                log.info("Change password request for user: {}", userId);
                return authServiceClient
                                .changePassword(token, userId, request.getOldPassword(), request.getNewPassword())
                                .then(Mono.just(ResponseEntity.ok().<Void>build()))
                                .onErrorResume(e -> {
